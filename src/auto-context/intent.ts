...
const HIGH_VALUE_PREFIXES = ["docs/", "plans/", "skills/", "graphify-out/"];
...
export function classifyGraphifyIntent(
	...
	// Architecture-question: input or result contains trigger patterns
	...
	// Settings or PrimeCodex surface terms
	const surfaceTerms = [
		"settings", "prime-settings", "tool", "command", "skill",
		"keybinding", "resources_discover",
	];
	for (const term of surfaceTerms) {
		if (inputText.includes(term)) {
			return {
				kind: "settings-or-surface",
				confidence: 0.6,
				reason: `Input mentions surface: ${term}`,
				suggestedQuestion: `How does ${term} relate to other system surfaces?`,
				cacheKey,
			};
		}
	}
